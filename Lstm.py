...
hidden_size = 128   # 向量維度更高
num_layers = 1  # 疊 幾 層 LSTM
seq_length = 10   # 一次輸入更多上下文
learning_rate = 0.005

# 建立資料集 
def make_dataset(text, seq_length):
    x_data, y_data = [], []
...
x_data, y_data = make_dataset(text, seq_length)
...
class TextLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers):
        ...
        self.embed = nn.Embedding(input_size, hidden_size) #建立一個「詞向量表」，每個字（根據編號）會被對應到一個向量
        self.lstm = nn.LSTM(hidden_size, hidden_size, num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, input_size)

    def forward(self, x):
        x = self.embed(x)#把輸入的數字序列（例如「我」→ 0，「喜」→ 1）轉換成向量
        out, _ = self.lstm(x)
        out = self.fc(out)
        return out

#  模型訓練 
model = TextLSTM(input_size, hidden_size, num_layers)
...
#  測試產生文字 
start = text[:seq_length]
input_test = torch.LongTensor([[char2idx[c] for c in start]])
result = start

model.eval()
with torch.no_grad():
    for _ in range(100):  # 產生100字
